...
- PostgreSQL 17;
- Flyway;
- Apache Kafka в KRaft-режиме и Spring Kafka;
- Kafka UI;
- Docker Compose;
- Lombok;
- Maven Wrapper;
...
- свободные TCP-порты `8080`, `8081`, `9092` и `5432`.
...
Kafka UI — по адресу [http://localhost:8081](http://localhost:8081). Для начала работы
откройте [страницу регистрации](http://localhost:8080/register), создайте пользователя и
войдите с указанными данными.

## События заказов

Приложение создаёт топик `orders.events` с тремя партициями. JSON-события
`OrderCreated`, `OrderCancelled` и `OrderStatusChanged` сначала сохраняются вместе с
изменением заказа в таблице `outbox_events`. Фоновый publisher отправляет
неопубликованные записи и заполняет `published_at` только после подтверждения Kafka.
При временной недоступности брокера запись остаётся в outbox и отправляется повторно.
Kafka key — UUID заказа, текущая версия контракта — `1`.
Сообщения можно открыть в Kafka UI: `local` → `Topics` → `orders.events` → `Messages`.

Опубликованные записи outbox по умолчанию хранятся семь дней и очищаются ежедневно.
Доставка имеет семантику at-least-once: потребители должны использовать `eventId` для
идемпотентной обработки возможных повторов.
...
- `KAFKA_BOOTSTRAP_SERVERS` — адрес брокера для приложения, по умолчанию `localhost:9092`;
- `KAFKA_PORT` — внешний порт Kafka, по умолчанию `9092`;
- `KAFKA_UI_PORT` — внешний порт Kafka UI, по умолчанию `8081`;
- `KAFKA_ORDERS_TOPIC` — топик событий заказов, по умолчанию `orders.events`;
- `OUTBOX_BATCH_SIZE` — максимальное число событий в одной выборке publisher;
- `OUTBOX_PUBLISH_INTERVAL` — интервал опроса outbox, по умолчанию `1s`;
- `OUTBOX_SEND_TIMEOUT` — ожидание подтверждения Kafka, по умолчанию `10s`;
- `OUTBOX_RETENTION` — срок хранения опубликованных событий, по умолчанию `7d`;
- `OUTBOX_CLEANUP_CRON` — расписание очистки, по умолчанию ежедневно в 03:00.
...
Flyway создаёт базовую схему миграциями `V1`–`V4`, загружает локальные ингредиенты
через `V100`, расширяет модель заказа миграцией `V101`, добавляет роли пользователей
миграцией `V102` и transactional outbox миграцией `V103`. Hibernate работает с
`ddl-auto: validate` и только проверяет соответствие JPA-модели созданной схеме.
...
- Kafka работает в KRaft-режиме, а события заказов публикуются в `orders.events` в версионированном JSON-конверте.
- Transactional outbox сохраняет заказ и событие атомарно и повторяет доставку после восстановления Kafka.
