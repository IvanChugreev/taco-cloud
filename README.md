...
Приложение создаёт топик `orders.events` с тремя партициями. После успешного коммита
транзакции в него публикуются JSON-события `OrderCreated`, `OrderCancelled` и
`OrderStatusChanged`. Kafka key — UUID заказа, текущая версия контракта — `1`.
